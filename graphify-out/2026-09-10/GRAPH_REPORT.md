...
## Corpus Check
- Corpus is ~1,356 words - fits in a single context window. You may not need a graph.
...
2. `GitHub Issue Tracker` - 3 edges
3. `Cube Plugin` - 3 edges
4. `Agent Skills Instructions` - 2 edges
5. `Domain Documentation Rules` - 2 edges
6. `Codex Marketplace` - 2 edges
7. `Claude Marketplace` - 2 edges
8. `Canonical Triage Roles` - 1 edges
9. `Reusable Agent Skills` - 1 edges
10. `Plugin Installation` - 1 edges
...
- `Agent Skills Instructions` --references--> `Domain Documentation Rules`  [EXTRACTED]
...
## Knowledge Gaps
- **6 isolated node(s):** `Agent Skills Guidance`, `Canonical Triage Roles`, `Reusable Agent Skills`, `Plugin Installation`, `GitHub Issue Operations` (+1 more)
...
- **What connects `Agent Skills Guidance`, `Canonical Triage Roles`, `Reusable Agent Skills` to the rest of the system?**
